...
DECLARE  v_fct_consolidado 																			VARCHAR(12) ;
  DECLARE v_fct_periodo 																			    INT(6);
  DECLARE v_fct_fecha_periodo  																			date;
  DECLARE v_fct_contador  																			    INTEGER DEFAULT 1;
  DECLARE v_fct_i  																			 			INTEGER DEFAULT 1;
...
-- SVSB
 ...
 DECLARE c_fecha CURSOR FOR
   select FCT_C_FECHA
    ...
    where FCT_C_CONS_ID = v_fct_c_cons_id;





DECLARE c_flujo_caja_terreno
...
SELECT CONS.CONS_ID ,
       ...
       SUM(CONS.FCT_C_FLUJO_ACUMULADO) FROM
  (SELECT CONS_ID 
  ...
  ,SUM(FCT_FLUJO_ACUMULADO) 									FCT_C_FLUJO_ACUMULADO
   ...
   WHERE SEL_DET_PRO_ID IS NOT NULL
     ...
     AND FCT_FECHA BETWEEN CASE CONS_FEC_GEN_DES
                           ...
                           END AND CASE CONS_FEC_GEN_HAS
                                       WHEN '0000-00-00' THEN FCT_FECHA
                                       ELSE CONS_FEC_GEN_HAS
                                   END
   GROUP BY FCT_FECHA
   UNION ALL SELECT CONS_ID 
   ,FCT_C_FECHA FCT_C_FECHA ,SUM(
   FCT_C_VALOR_ADQUISICION_PAGOS) 								FCT_C_VALOR_ADQUISICION_PAGOS 
   ...
   FROM tb_seleccion ,tb_c_flujo_caja_terreno ,tb_consolidados
...
obtener_c_flujo_caja_terreno: LOOP FETCH C_FLUJO_CAJA_TERRENO INTO v_fct_c_cons_id,
                                                                   v_fecha,
...
IF v_finished = 1 THEN LEAVE obtener_c_flujo_caja_terreno;
...
-- SVSB
SET v_finished = 0;
-- SET v_fct_consolidado = v_fct_cons_id; 

select cons_per
  into v_fct_periodo
  from tb_consolidados
 where cons_id = v_fct_c_cons_id;

OPEN c_fecha;
 obtener_fecha:LOOP
  FETCH c_fecha into v_fct_fecha_periodo;
  
  IF v_finished = 1 THEN
     LEAVE obtener_fecha;
  END IF;   
  
  UPDATE TB_C_FLUJO_CAJA_TERRENO
     SET FCT_C_PERIODICIDAD = v_fct_contador
   WHERE FCT_C_FECHA = v_fct_fecha_periodo
     AND FCT_C_CONS_ID = v_fct_c_cons_id;
  
  if v_fct_i = v_fct_periodo then
   SET v_fct_contador = v_fct_contador + 1;
   SET v_fct_i = 0;
  end if;
  SET v_fct_i = v_fct_i+1;  
