...
<div style="position:absolute; left:10%; top:0px; width:1024px; height:768px;">
	...
	<div id="p2" onclick="location.href='../Carga/ABT-T011-carga-U.php';" style="background-image:url(carga-manual_03.png);" title="">
	</div>
	<div style="background-image:url(carga-manual_04.png); position:absolute; left:802px; top:41px; width:21px; height:727px;" title="">
	</div>
	<div id="p3" onclick="location.href='../Bienvenido/ABT-T010-bienvenido.php';" style="background-image:url(carga-manual_05.png); " title="">
	</div>
				...
				if(isset($_POST['radio'])){
					...
					if($resultado_tb->num_rows <1 ){
							...
							if(!$loadedSheetNames[0]=="Flujo Caja Terreno"){
								...
								print "<script>alert(\"Error en el momento de encontrar la hoja Flujo Caja Terreno\");</script>";
								exit;
							}
							
							if(!$loadedSheetNames[1]=="Flujo Caja"){
								
								print "<script>alert(\"Error en el momento de encontrar la hoja Flujo Caja\");</script>";
								exit;
							}
							
							if(!$loadedSheetNames[2]=="Resumen"){
								
								print "<script>alert(\"Error en el momento de encontrar la hoja Resumen\");</script>";
								exit;
							}
							
							if(!$loadedSheetNames[4]=="Otros Flujos"){
								
								print "<script>alert(\"Error en el momento de encontrar la hoja Otros Flujos\");</script>";
								exit;
							}
						...
						//	echo('There was an error running the query [' . $mysqli->error . '].</br>');
							...
							//---------Inicio Agregar datos		399	
							
							//Ingreso de la informacion teniendo en cuenta las distintas hojas
							//--------------------------Flujo Caja Terreno
							FlujoCajaTerreno($objPHPExcel,$proyecto,$etapa,$modelo,$mysqli,$error_insert);
							//--------------------------------FLUJO CAJA-----------------------
							FlujoCaja($objPHPExcel,$flc_i,$proyecto,$etapa,$modelo,$mysqli,$error_insert);
							//----------------------------------RESUMEN---------------------
							Resumen($objPHPExcel,$check_model,$proyecto,$etapa,$modelo,$mysqli,$error_insert);
							//---------------------OTROS FLUJOS-----------------------
							OtrosFlujos($objPHPExcel,$ofl_i,$ofl_var,$proyecto,$etapa,$modelo,$mysqli,$error_insert);	
							//------------Fin ingreso datos 
					...
					print "<script>alert(\"Error en el Tipo de archivo\");</script>";
				}
		}
		else{
					print "<script>alert(\"Error en el Formato del nombre no es correcto\");</script>";
				}
		...
		function cambiarFecha($fecha){
				...
				function Resumen($objPHPExcel,$check_model,$proyecto,$etapa,$modelo,$mysqli,$error_insert){
					...
					//Toma la informacion cenda por celda
							...
							$r_check=$objWorksheet->getCellByColumnAndRow(2, 22)->getFormattedValue();
							$r_check2=cambiarFecha($r_check);
								
							$res_fecha_presupuesto=$r_check2;
