  ...
  include "../../php/ABT-T000-conexion-U.php";
	...
	$Example=$_SESSION['VAR']; 
		...
		OFL_C_PERIODICIDAD,
			 ...
			 CASE WHEN SUM(OFL_C_GO_PROMOTORA_OPERACION)  = 0 THEN '' ELSE (SUM(OFL_C_GO_PROMOTORA_OPERACION) *  CONS_UNI_PRE) END OFL_C_GO_PROMOTORA_OPERACION,
			 CASE WHEN SUM(OFL_C_GO_PROMOTORA_OVERHEAD)  = 0 THEN '' ELSE (SUM(OFL_C_GO_PROMOTORA_OVERHEAD) *  CONS_UNI_PRE) END OFL_C_GO_PROMOTORA_OVERHEAD,
			 CASE WHEN SUM(OFL_C_GO_CONSTRUCTORA_OPERACION)  = 0 THEN '' ELSE (SUM(OFL_C_GO_CONSTRUCTORA_OPERACION) *  CONS_UNI_PRE) END OFL_C_GO_CONSTRUCTORA_OPERACION,
			 CASE WHEN SUM(OFL_C_GO_CONSTRUCTORA_OVERHEAD)  = 0 THEN '' ELSE (SUM(OFL_C_GO_CONSTRUCTORA_OVERHEAD) *  CONS_UNI_PRE) END OFL_C_GO_CONSTRUCTORA_OVERHEAD,
			 CASE WHEN SUM(OFL_C_GO_TRANSACCIONES_INMOBILIARIAS_OPERACION)  = 0 THEN '' ELSE (SUM(OFL_C_GO_TRANSACCIONES_INMOBILIARIAS_OPERACION) *  CONS_UNI_PRE) END OFL_C_GO_TRANSACCIONES_INMOBILIARIAS_OPERACION,
			 CASE WHEN SUM(OFL_C_GO_TRANSACCIONES_INMOBLIARIAS_OVERHEAD)  = 0 THEN '' ELSE (SUM(OFL_C_GO_TRANSACCIONES_INMOBLIARIAS_OVERHEAD) *  CONS_UNI_PRE) END OFL_C_GO_TRANSACCIONES_INMOBLIARIAS_OVERHEAD,
			 CASE WHEN SUM(OFL_C_OTROS_PAGOS_EFECTUADOS_POR_FIDUCIA)  = 0 THEN '' ELSE (SUM(OFL_C_OTROS_PAGOS_EFECTUADOS_POR_FIDUCIA) *  CONS_UNI_PRE) END OFL_C_OTROS_PAGOS_EFECTUADOS_POR_FIDUCIA,
			 CASE WHEN SUM(OFL_C_VALOR_TERRENO)  = 0 THEN '' ELSE (SUM(OFL_C_VALOR_TERRENO) *  CONS_UNI_PRE) END OFL_C_VALOR_TERRENO,
			 CASE WHEN SUM(OFL_C_EGRESOS_CREDITO_CONSTRUCTOR)  = 0 THEN '' ELSE (SUM(OFL_C_EGRESOS_CREDITO_CONSTRUCTOR) *  CONS_UNI_PRE) END OFL_C_EGRESOS_CREDITO_CONSTRUCTOR,
			 CASE WHEN SUM(OFL_C_IMPUESTO_TRANSACCIONES_FINANCIERAS_COLPATRIA)  = 0 THEN '' ELSE (SUM(OFL_C_IMPUESTO_TRANSACCIONES_FINANCIERAS_COLPATRIA) *  CONS_UNI_PRE) END OFL_C_IMPUESTO_TRANSACCIONES_FINANCIERAS_COLPATRIA,
			 CASE WHEN SUM(OFL_C_COMISION_FIDUCIARIA_NUEVOGAR_SIN_FIDUCIARIA)  = 0 THEN '' ELSE (SUM(OFL_C_COMISION_FIDUCIARIA_NUEVOGAR_SIN_FIDUCIARIA) *  CONS_UNI_PRE) END OFL_C_COMISION_FIDUCIARIA_NUEVOGAR_SIN_FIDUCIARIA,
			 CASE WHEN SUM(OFL_C_OTROS_COSTOS_Y_GASTOS)  = 0 THEN '' ELSE (SUM(OFL_C_OTROS_COSTOS_Y_GASTOS) *  CONS_UNI_PRE) END OFL_C_OTROS_COSTOS_Y_GASTOS,
			 CASE WHEN SUM(OFL_C_APORTES_EFECTIVO_COLPATRIA_FIDEICOMISOS)  = 0 THEN '' ELSE (SUM(OFL_C_APORTES_EFECTIVO_COLPATRIA_FIDEICOMISOS) *  CONS_UNI_PRE) END OFL_C_APORTES_EFECTIVO_COLPATRIA_FIDEICOMISOS,
			 CASE WHEN SUM(OFL_C_REINTEGROS_IVA)  = 0 THEN '' ELSE (SUM(OFL_C_REINTEGROS_IVA) *  CONS_UNI_PRE) END OFL_C_REINTEGROS_IVA,
			 CASE WHEN SUM(OFL_C_SUBTOTAL_EGRESOS_COLPATRIA)  = 0 THEN '' ELSE (SUM(OFL_C_SUBTOTAL_EGRESOS_COLPATRIA) *  CONS_UNI_PRE) END OFL_C_SUBTOTAL_EGRESOS_COLPATRIA,
			 CASE WHEN SUM(OFL_C_TRASLADOS_FIDUCIAS_E_INGRESOS_TOTALES)  = 0 THEN '' ELSE (SUM(OFL_C_TRASLADOS_FIDUCIAS_E_INGRESOS_TOTALES) *  CONS_UNI_PRE) END OFL_C_TRASLADOS_FIDUCIAS_E_INGRESOS_TOTALES,
			 CASE WHEN SUM(OFL_C_INGRESOS_VENTAS_COLPATRIA)  = 0 THEN '' ELSE (SUM(OFL_C_INGRESOS_VENTAS_COLPATRIA) *  CONS_UNI_PRE) END OFL_C_INGRESOS_VENTAS_COLPATRIA,
			 CASE WHEN SUM(OFL_C_DESEMBOLSOS_CREDITO_CONSTRUCTOR_TERRENO)  = 0 THEN '' ELSE (SUM(OFL_C_DESEMBOLSOS_CREDITO_CONSTRUCTOR_TERRENO) *  CONS_UNI_PRE) END OFL_C_DESEMBOLSOS_CREDITO_CONSTRUCTOR_TERRENO,
			 CASE WHEN SUM(OFL_C_TRASLADOS_LIQUIDACIONES_FIDEICOMISOS)  = 0 THEN '' ELSE (SUM(OFL_C_TRASLADOS_LIQUIDACIONES_FIDEICOMISOS) *  CONS_UNI_PRE) END OFL_C_TRASLADOS_LIQUIDACIONES_FIDEICOMISOS,
			 CASE WHEN SUM(OFL_C_OTROS_INGRESOS)  = 0 THEN '' ELSE (SUM(OFL_C_OTROS_INGRESOS) *  CONS_UNI_PRE) END OFL_C_OTROS_INGRESOS,
			 CASE WHEN SUM(OFL_C_APORTES_DE_SOCIOS)  = 0 THEN '' ELSE (SUM(OFL_C_APORTES_DE_SOCIOS) *  CONS_UNI_PRE) END OFL_C_APORTES_DE_SOCIOS,
			 CASE WHEN SUM(OFL_C_SUBTOTAL_INGRESOS)  = 0 THEN '' ELSE (SUM(OFL_C_SUBTOTAL_INGRESOS) *  CONS_UNI_PRE) END OFL_C_SUBTOTAL_INGRESOS,
			 CASE WHEN SUM(OFL_C_FLUJO_CAJA_ANTES_DE_REINTEGRO_SOCIOS)  = 0 THEN '' ELSE (SUM(OFL_C_FLUJO_CAJA_ANTES_DE_REINTEGRO_SOCIOS) *  CONS_UNI_PRE) END OFL_C_FLUJO_CAJA_ANTES_DE_REINTEGRO_SOCIOS,
			 CASE WHEN SUM(OFL_C_REINTEGRO_APORTES_UTILIDADES_SOCIOS)  = 0 THEN '' ELSE (SUM(OFL_C_REINTEGRO_APORTES_UTILIDADES_SOCIOS) *  CONS_UNI_PRE) END OFL_C_REINTEGRO_APORTES_UTILIDADES_SOCIOS,
			 CASE WHEN SUM(OFL_C_FLUJO_NETO_CAJA_CT)  = 0 THEN '' ELSE (SUM(OFL_C_FLUJO_NETO_CAJA_CT) *  CONS_UNI_PRE) END OFL_C_FLUJO_NETO_CAJA_CT,
			 CASE WHEN SUM(OFL_C_FLUJO_ACUMULADO)  = 0 THEN '' ELSE (SUM(OFL_C_FLUJO_ACUMULADO) *  CONS_UNI_PRE) END OFL_C_FLUJO_ACUMULADO,
			 CASE WHEN SUM(OFL_C_COSTO_CREDITO_TESORERIA)  = 0 THEN '' ELSE (SUM(OFL_C_COSTO_CREDITO_TESORERIA) *  CONS_UNI_PRE) END OFL_C_COSTO_CREDITO_TESORERIA,
			 CASE WHEN SUM(OFL_C_IMPUESTO_RENTA)  = 0 THEN '' ELSE (SUM(OFL_C_IMPUESTO_RENTA) *  CONS_UNI_PRE) END OFL_C_IMPUESTO_RENTA,
			 CASE WHEN SUM(OFL_C_FLUJO_NETO_CAJA)  = 0 THEN '' ELSE (SUM(OFL_C_FLUJO_NETO_CAJA) *  CONS_UNI_PRE) END OFL_C_FLUJO_NETO_CAJA,
			 CASE WHEN SUM(OFL_C_FLUJO_ACUMULADO_CAJA)  = 0 THEN '' ELSE (SUM(OFL_C_FLUJO_ACUMULADO_CAJA) *  CONS_UNI_PRE) END OFL_C_FLUJO_ACUMULADO_CAJA,
			 CASE WHEN SUM(OFL_C_VPN)  = 0 THEN '' ELSE (SUM(OFL_C_VPN) *  CONS_UNI_PRE) END OFL_C_VPN,
			 CASE WHEN SUM(OFL_C_TIR_EA)  = 0 THEN '' ELSE (SUM(OFL_C_TIR_EA) *  CONS_UNI_PRE) END OFL_C_TIR_EA,
			 CASE WHEN SUM(OFL_C_TIR_EM)  = 0 THEN '' ELSE (SUM(OFL_C_TIR_EM) *  CONS_UNI_PRE) END OFL_C_TIR_EM,
			 CASE WHEN SUM(OFL_C_TIR_MOD_EA)  = 0 THEN '' ELSE (SUM(OFL_C_TIR_MOD_EA) *  CONS_UNI_PRE) END OFL_C_TIR_MOD_EA,
			 CASE WHEN SUM(OFL_C_TIR_MOD_EM)  = 0 THEN '' ELSE (SUM(OFL_C_TIR_MOD_EM) *  CONS_UNI_PRE) END OFL_C_TIR_MOD_EM,
			 CASE WHEN SUM(OFL_C_VALOR_TOTAL_VENTAS)  = 0 THEN '' ELSE (SUM(OFL_C_VALOR_TOTAL_VENTAS) *  CONS_UNI_PRE) END OFL_C_VALOR_TOTAL_VENTAS,
			 CASE WHEN SUM(OFL_C_PAGOS_EFECTUADOS_FIDUCIA_ADMON)  = 0 THEN '' ELSE (SUM(OFL_C_PAGOS_EFECTUADOS_FIDUCIA_ADMON) *  CONS_UNI_PRE) END OFL_C_PAGOS_EFECTUADOS_FIDUCIA_ADMON,
			 CASE WHEN SUM(OFL_C_PE_INTERESES_CREDITO_CONSTRUCTOR)  = 0 THEN '' ELSE (SUM(OFL_C_PE_INTERESES_CREDITO_CONSTRUCTOR) *  CONS_UNI_PRE) END OFL_C_PE_INTERESES_CREDITO_CONSTRUCTOR,
			 CASE WHEN SUM(OFL_C_PE_HONORARIOS_VISITAS_OBRA_OTROS)  = 0 THEN '' ELSE (SUM(OFL_C_PE_HONORARIOS_VISITAS_OBRA_OTROS) *  CONS_UNI_PRE) END OFL_C_PE_HONORARIOS_VISITAS_OBRA_OTROS,
			 CASE WHEN SUM(OFL_C_PE_ABONOS_EXTRAORDINARIOS_CREDITO)  = 0 THEN '' ELSE (SUM(OFL_C_PE_ABONOS_EXTRAORDINARIOS_CREDITO) *  CONS_UNI_PRE) END OFL_C_PE_ABONOS_EXTRAORDINARIOS_CREDITO,
			 CASE WHEN SUM(OFL_C_PE_PAGOS_TERRENO_EFECTUADOS_FIDUCIA)  = 0 THEN '' ELSE (SUM(OFL_C_PE_PAGOS_TERRENO_EFECTUADOS_FIDUCIA) *  CONS_UNI_PRE) END OFL_C_PE_PAGOS_TERRENO_EFECTUADOS_FIDUCIA,
			 CASE WHEN SUM(OFL_C_PE_OTROS_PAGOS_EFECTUADOS_FIDUCIA)  = 0 THEN '' ELSE (SUM(OFL_C_PE_OTROS_PAGOS_EFECTUADOS_FIDUCIA) *  CONS_UNI_PRE) END OFL_C_PE_OTROS_PAGOS_EFECTUADOS_FIDUCIA,
			 CASE WHEN SUM(OFL_C_TRASLADOS_FONDOS_FIDUCIA_CONSTRUCTORA)  = 0 THEN '' ELSE (SUM(OFL_C_TRASLADOS_FONDOS_FIDUCIA_CONSTRUCTORA) *  CONS_UNI_PRE) END OFL_C_TRASLADOS_FONDOS_FIDUCIA_CONSTRUCTORA,
			 CASE WHEN SUM(OFL_C_COMISION_FIDUCIARIA_PRREVENTAS_ADMON)  = 0 THEN '' ELSE (SUM(OFL_C_COMISION_FIDUCIARIA_PRREVENTAS_ADMON) *  CONS_UNI_PRE) END OFL_C_COMISION_FIDUCIARIA_PRREVENTAS_ADMON,
			 CASE WHEN SUM(OFL_C_COMISION_FIDUCIARIA_NUEVOGAR)  = 0 THEN '' ELSE (SUM(OFL_C_COMISION_FIDUCIARIA_NUEVOGAR) *  CONS_UNI_PRE) END OFL_C_COMISION_FIDUCIARIA_NUEVOGAR,
			 CASE WHEN SUM(OFL_C_IMPUESTO_TRANSACCIONES_FINANCIERAS)  = 0 THEN '' ELSE (SUM(OFL_C_IMPUESTO_TRANSACCIONES_FINANCIERAS) *  CONS_UNI_PRE) END OFL_C_IMPUESTO_TRANSACCIONES_FINANCIERAS,
			 CASE WHEN SUM(OFL_C_SUBTOTAL_EGRESOS)  = 0 THEN '' ELSE (SUM(OFL_C_SUBTOTAL_EGRESOS) *  CONS_UNI_PRE) END OFL_C_SUBTOTAL_EGRESOS,
			 CASE WHEN SUM(OFL_C_DESEMBOLSOS_CREDITO_CONSTRUCTOR)  = 0 THEN '' ELSE (SUM(OFL_C_DESEMBOLSOS_CREDITO_CONSTRUCTOR) *  CONS_UNI_PRE) END OFL_C_DESEMBOLSOS_CREDITO_CONSTRUCTOR,
			 CASE WHEN SUM(OFL_C_INGRESOS_VENTAS)  = 0 THEN '' ELSE (SUM(OFL_C_INGRESOS_VENTAS) *  CONS_UNI_PRE) END OFL_C_INGRESOS_VENTAS,
			 CASE WHEN SUM(OFL_C_IV_ABONOS_SEPARACION)  = 0 THEN '' ELSE (SUM(OFL_C_IV_ABONOS_SEPARACION) *  CONS_UNI_PRE) END OFL_C_IV_ABONOS_SEPARACION,
			 CASE WHEN SUM(OFL_C_IV_CARTERA_CUOTA_INICIAL)  = 0 THEN '' ELSE (SUM(OFL_C_IV_CARTERA_CUOTA_INICIAL) *  CONS_UNI_PRE) END OFL_C_IV_CARTERA_CUOTA_INICIAL,
			 CASE WHEN SUM(OFL_C_IV_SALDO_CUOTA_INICIAL_ABONO_ESCRITURA)  = 0 THEN '' ELSE (SUM(OFL_C_IV_SALDO_CUOTA_INICIAL_ABONO_ESCRITURA) *  CONS_UNI_PRE) END OFL_C_IV_SALDO_CUOTA_INICIAL_ABONO_ESCRITURA,
			 CASE WHEN SUM(OFL_C_IV_SUBSIDIO_VIS_Y_AHORRO_PROGRAMADO)  = 0 THEN '' ELSE (SUM(OFL_C_IV_SUBSIDIO_VIS_Y_AHORRO_PROGRAMADO) *  CONS_UNI_PRE) END OFL_C_IV_SUBSIDIO_VIS_Y_AHORRO_PROGRAMADO,
			 CASE WHEN SUM(OFL_C_IV_EXCEDENTES_CREDITO_COMPRADORES)  = 0 THEN '' ELSE (SUM(OFL_C_IV_EXCEDENTES_CREDITO_COMPRADORES) *  CONS_UNI_PRE) END OFL_C_IV_EXCEDENTES_CREDITO_COMPRADORES,
			 CASE WHEN SUM(OFL_C_IV_GIROS_DIRECTOS_CREDITOS_OTRAS_ENTIDADES)  = 0 THEN '' ELSE (SUM(OFL_C_IV_GIROS_DIRECTOS_CREDITOS_OTRAS_ENTIDADES) *  CONS_UNI_PRE) END OFL_C_IV_GIROS_DIRECTOS_CREDITOS_OTRAS_ENTIDADES,
			 CASE WHEN SUM(OFL_C_IV_INGRESOS_POR_RECIBIR_VENTAS_REALIZADAS)  = 0 THEN '' ELSE (SUM(OFL_C_IV_INGRESOS_POR_RECIBIR_VENTAS_REALIZADAS) *  CONS_UNI_PRE) END OFL_C_IV_INGRESOS_POR_RECIBIR_VENTAS_REALIZADAS,
			 CASE WHEN SUM(OFL_C_OTROS_INGRESOS_POR_VENTAS)  = 0 THEN '' ELSE (SUM(OFL_C_OTROS_INGRESOS_POR_VENTAS) *  CONS_UNI_PRE) END OFL_C_OTROS_INGRESOS_POR_VENTAS,
			 CASE WHEN SUM(OFL_C_OI_INTERESES_SUBROGACION)  = 0 THEN '' ELSE (SUM(OFL_C_OI_INTERESES_SUBROGACION) *  CONS_UNI_PRE) END OFL_C_OI_INTERESES_SUBROGACION,
			 CASE WHEN SUM(OFL_C_OI_INTERESES_MORA)  = 0 THEN '' ELSE (SUM(OFL_C_OI_INTERESES_MORA) *  CONS_UNI_PRE) END OFL_C_OI_INTERESES_MORA,
			 CASE WHEN SUM(OFL_C_OI_OFERTAS_COMERCIALES_Y_OTROS_INGRESOS)  = 0 THEN '' ELSE (SUM(OFL_C_OI_OFERTAS_COMERCIALES_Y_OTROS_INGRESOS) *  CONS_UNI_PRE) END OFL_C_OI_OFERTAS_COMERCIALES_Y_OTROS_INGRESOS,
			 CASE WHEN SUM(OFL_C_APORTES_EFECTIVO_COLPATRIA_FIDEICOMISO)  = 0 THEN '' ELSE (SUM(OFL_C_APORTES_EFECTIVO_COLPATRIA_FIDEICOMISO) *  CONS_UNI_PRE) END OFL_C_APORTES_EFECTIVO_COLPATRIA_FIDEICOMISO,
			 CASE WHEN SUM(OFL_C_APORTES_OTROS_SOCIOS_INVERSIONISTAS)  = 0 THEN '' ELSE (SUM(OFL_C_APORTES_OTROS_SOCIOS_INVERSIONISTAS) *  CONS_UNI_PRE) END OFL_C_APORTES_OTROS_SOCIOS_INVERSIONISTAS,
			 CASE WHEN SUM(OFL_C_RENDIMIENTOS_FIDEICOMISO_CAPITALIZACION)  = 0 THEN '' ELSE (SUM(OFL_C_RENDIMIENTOS_FIDEICOMISO_CAPITALIZACION) *  CONS_UNI_PRE) END OFL_C_RENDIMIENTOS_FIDEICOMISO_CAPITALIZACION,
			 CASE WHEN SUM(OFL_C_SUBTOTAL_INGRESOS_ANTES_RENDIMIENTOS)  = 0 THEN '' ELSE (SUM(OFL_C_SUBTOTAL_INGRESOS_ANTES_RENDIMIENTOS) *  CONS_UNI_PRE) END OFL_C_SUBTOTAL_INGRESOS_ANTES_RENDIMIENTOS,
			 CASE WHEN SUM(OFL_C_FLUJO_NETO_SIN_RENDIMIENTOS)  = 0 THEN '' ELSE (SUM(OFL_C_FLUJO_NETO_SIN_RENDIMIENTOS) *  CONS_UNI_PRE) END OFL_C_FLUJO_NETO_SIN_RENDIMIENTOS,
			 CASE WHEN SUM(OFL_C_RENDIMIENTOS_FIDEICOMISO)  = 0 THEN '' ELSE (SUM(OFL_C_RENDIMIENTOS_FIDEICOMISO) *  CONS_UNI_PRE) END OFL_C_RENDIMIENTOS_FIDEICOMISO,
			 CASE WHEN SUM(OFL_C_FLUJO_NETO_FIDUCIARIA)  = 0 THEN '' ELSE (SUM(OFL_C_FLUJO_NETO_FIDUCIARIA) *  CONS_UNI_PRE) END OFL_C_FLUJO_NETO_FIDUCIARIA,
			 CASE WHEN SUM(OFL_C_TRASLADO_SALDO_FINAL_FIDUCIA_LIQUIDACION)  = 0 THEN '' ELSE (SUM(OFL_C_TRASLADO_SALDO_FINAL_FIDUCIA_LIQUIDACION) *  CONS_UNI_PRE) END OFL_C_TRASLADO_SALDO_FINAL_FIDUCIA_LIQUIDACION,
			 CASE WHEN SUM(OFL_C_FLUJO_ACUMULADO_FIDUCIARIA)  = 0 THEN '' ELSE (SUM(OFL_C_FLUJO_ACUMULADO_FIDUCIARIA) *  CONS_UNI_PRE) END OFL_C_FLUJO_ACUMULADO_FIDUCIARIA    
